    ...
    var body: some View {
        Group {
            if let profile = profile {
                                ...
                                ForEach(profileEvents, id: \.id) { event in
                                    Button(action: {
                                        ...
                                        HStack {
                                            ...
                                            Text(
                                                DateFormatterHelper.formatEventDate(
                                                    event.nextDate,
                                                    locale: localizationManager.currentLanguage.locale
                                                )
                                            )
                ...
                .navigationDestination(isPresented: $viewModel.showingAddGift) {
                    AddEditGiftView(profileId: profile.id)
                }
                .navigationDestination(item: $viewModel.showingEditGift) { gift in
                ...
                .onChange(of: viewModel.showingAddGift) { _, newValue in
                    if newValue { viewModel.navigatingToChild = true }
                }
                .onChange(of: viewModel.showingEditGift) { _, newValue in
                    if newValue != nil { viewModel.navigatingToChild = true }
                }
                .onChange(of: viewModel.showingAddEvent) { _, newValue in
                    if newValue { viewModel.navigatingToChild = true }
                }
                .onChange(of: viewModel.showingEditEvent) { _, newValue in
                    if newValue != nil { viewModel.navigatingToChild = true }
                }
                .onChange(of: allProfiles) { _, newValue in
